...
| Redis fail-closed chaos (CI) | Security P0 | **Code/CI Ready** | Engineering | Jest fail-closed + CI docker-stop probe | `edge-replay-fail-closed.test.ts` + `redis-chaos` job | pending merge |
| Production forbids SIMULATION | Safety P0 | **Code Ready** | Engineering | `ALLOW_SIMULATION_IN_PRODUCTION` gate + Render `PILOT_LIVE` | `env.ts` + `render.yaml` | pending merge |
...
| Observability fire drill / issue #50 | Ops P0 | **Partial** | Ops / before staging | Live probes Done; catalog ownership + dashboard/drain stubs Done; local webhook dispatcher PASS; Render `ALERT_WEBHOOK_*` deliver/ack still Open | SHA-256 `c50405a5897c98fb564fabf43511d2a2cc598b2d72376d76eed449030dceb4b5` | |
