...
import {
  acknowledgeAlarmEvent, appendIncidentTimeline, createAlarmRule, createIncident,
  listAlarmEvents, listAlarmRules, listIncidents, raiseAlarmEvent
...
export const postAlarmRule = async (req: Request, res: Response) => {
  ...
  const payload: Parameters<typeof createAlarmRule>[0] = {
    organisationId: String(b.organisationId), name: String(b.name), metricKey: String(b.metricKey), threshold: Number(b.threshold)
  };
  ...
  if (b.comparator) payload.comparator = String(b.comparator) as NonNullable<Parameters<typeof createAlarmRule>[0]["comparator"]>;
  if (b.severity) payload.severity = String(b.severity) as NonNullable<Parameters<typeof createAlarmRule>[0]["severity"]>;
...
export const postAlarmEvent = async (req: Request, res: Response) => {
  ...
  const payload: Parameters<typeof raiseAlarmEvent>[0] = {
    organisationId: String(b.organisationId), siteId: String(b.siteId), title: String(b.title), message: String(b.message)
  };
  if (b.plantId) payload.plantId = String(b.plantId);
  if (b.assetId) payload.assetId = String(b.assetId);
  if (b.ruleId) payload.ruleId = String(b.ruleId);
  if (b.severity) payload.severity = String(b.severity) as NonNullable<Parameters<typeof raiseAlarmEvent>[0]["severity"]>;
...
export const postAcknowledgeAlarm = async (req: Request, res: Response) => {
  ...
  res.status(201).json({ data: await acknowledgeAlarmEvent(alarmEventId, actorFrom(req), b.note ? String(b.note) : undefined) });
};
...
export const postIncident = async (req: Request, res: Response) => {
  ...
  const payload: Parameters<typeof createIncident>[0] = {
    organisationId: String(b.organisationId), siteId: String(b.siteId), title: String(b.title)
  };
  if (b.plantId) payload.plantId = String(b.plantId);
  if (b.summary) payload.summary = String(b.summary);
  if (b.severity) payload.severity = String(b.severity) as NonNullable<Parameters<typeof createIncident>[0]["severity"]>;
...
export const postIncidentTimeline = async (req: Request, res: Response) => {
  ...
  res.status(201).json({ data: await appendIncidentTimeline(incidentId, actorFrom(req), {
    eventType: String(b.eventType), message: String(b.message), metadata: b.metadata as never
  }) });
};
